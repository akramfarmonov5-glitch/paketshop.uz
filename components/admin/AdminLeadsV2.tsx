...
export default function AdminLeadsV2() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-900">Leadlar (CRM)</h2>
        <p className="mt-1 text-sm text-slate-500">Ulgurji, tashkilot va kontakt so‘rovlari — status, mas‘ul menejer va faoliyat tarixi bilan.</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="relative ml-auto">
          ...
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Ism yoki telefon" className="h-10 rounded-xl border border-slate-200 bg-white/40 pl-9 pr-3 text-sm text-slate-900 outline-none focus:border-red-600" />
        </div>
      </div>
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
        {loading ? (
                ...
                <React.Fragment key={lead.id}>
                  <tr className="border-b border-slate-100">
                    <td className="px-4 py-3">
                      ...
                      <a href={`tel:${lead.phone}`} className="text-xs text-slate-500 hover:text-red-600">{lead.phone}</a>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <select value={lead.status} onChange={(event) => handleStatusChange(lead, event.target.value)} aria-label={`${lead.name} statusi`} className={`rounded-lg border-0 px-2 py-1.5 text-xs font-bold outline-none ${STATUS_TONES[lead.status] || 'bg-slate-50 text-slate-600'}`}>
                        {Object.entries(STATUS_LABELS).map(([status, label]) => <option key={status} value={status} className="bg-white text-slate-900">{label}</option>)}
                      </select>
                    ...
                    <td className="px-4 py-3">
                      <select value={lead.assignedTo?.id || ''} onChange={(event) => patchLead(lead.id, { assignedToId: event.target.value || null })} aria-label={`${lead.name} mas'uli`} className="max-w-[160px] rounded-lg border border-slate-200 bg-white/40 px-2 py-1.5 text-xs text-slate-700 outline-none">
                        <option value="">—</option>
                        {managers.map((manager) => <option key={manager.id} value={manager.id}>{manager.name || manager.email}</option>)}
                      </select>
                    </td>
                    <td className="px-4 py-3 text-xs text-slate-500">{new Date(lead.createdAt).toLocaleDateString('uz-UZ')}</td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => { setExpandedId(expandedId === lead.id ? null : lead.id); setNoteDraft(''); }} aria-label="Tafsilotlar" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100">
                    ...
                    <tr className="border-b border-slate-100 bg-white/30">
                      <td colSpan={7} className="px-6 py-4">
                        <div className="grid gap-6 md:grid-cols-2">
                          <div>
                            <h4 className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">So‘rov tafsilotlari</h4>
                            {payloadEntries(lead).length ? (
                              <dl className="space-y-1.5 text-sm">
                                {payloadEntries(lead).map(([label, value]) => (
                                  <div key={label} className="flex gap-2"><dt className="w-32 shrink-0 text-slate-400">{label}:</dt><dd className="text-slate-700">{value}</dd></div>
                                ))}
                              </dl>
                            ) : <p className="text-sm text-slate-400">Qo‘shimcha ma’lumot yo‘q</p>}
                          </div>
                          <div>
                            <h4 className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">Faoliyat tarixi</h4>
                            <ul className="max-h-48 space-y-2 overflow-y-auto text-sm">
                              {lead.activities.map((activity) => (
                                <li key={activity.id} className="rounded-lg bg-slate-50 px-3 py-2">
                                  <p className="text-slate-700">{activity.note || activity.type}</p>
                                  <p className="mt-0.5 text-[11px] text-slate-400">{new Date(activity.createdAt).toLocaleString('uz-UZ')}</p>
                                </li>
                              ))}
                            </ul>
          ...
          <div className="py-16 text-center text-slate-400">Leadlar topilmadi</div>
        )}
      </div>
    </div>
  );
