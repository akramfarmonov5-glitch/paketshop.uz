...
export default function AdminOrders() {
  ...
  return <div className="space-y-6">
    <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h2 className="text-3xl font-bold text-white">Buyurtmalar (CRM)</h2><p className="mt-2 text-gray-400">Prisma orderlari, menejer va to‘liq status tarixi.</p></div><div className="flex gap-2"><button onClick={() => void load(query, filterStatus)} className="inline-flex items-center gap-2 rounded-xl border border-white/10 px-4 py-2 text-white"><RefreshCw size={17} />Yangilash</button><button onClick={exportCsv} disabled={!orders.length} className="inline-flex items-center gap-2 rounded-xl bg-gold-400 px-4 py-2 font-bold text-black disabled:opacity-50"><Download size={17} />CSV</button></div></header>
    <form onSubmit={(event) => { event.preventDefault(); void load(query, filterStatus); }} className="grid gap-3 sm:grid-cols-[1fr_240px_auto]"><label className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" size={19} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Raqam, mijoz yoki telefon…" className="w-full rounded-xl border border-white/10 bg-zinc-900 py-3 pl-12 pr-4 text-white outline-none" /></label><select value={filterStatus} onChange={(event) => setFilterStatus(event.target.value)} className="rounded-xl border border-white/10 bg-zinc-900 px-4 text-white"><option value="">Barcha holatlar</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><button className="rounded-xl bg-white/10 px-5 font-semibold text-white">Qidirish</button></form>
    {error && <p className="rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-red-300">{error}</p>}
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-zinc-900"><div className="overflow-x-auto"><table className="w-full min-w-[1000px] text-left text-sm"><thead className="border-b border-white/5 bg-white/5 text-gray-400"><tr><th className="p-4">Raqam / sana</th><th className="p-4">Mijoz</th><th className="p-4">Mahsulotlar</th><th className="p-4">Summa</th><th className="p-4">Menejer</th><th className="p-4">Holat</th></tr></thead><tbody className="divide-y divide-white/5">
      {loading ? <tr><td colSpan={6} className="p-12 text-center"><Loader2 className="mx-auto animate-spin text-gray-400" /></td></tr> : !orders.length ? <tr><td colSpan={6} className="p-12 text-center text-gray-500">Buyurtmalar topilmadi.</td></tr> : orders.map((order) => <tr key={order.id} className="align-top hover:bg-white/[.03]"><td className="p-4"><p className="font-mono font-semibold text-white">{order.number}</p><p className="mt-1 text-xs text-gray-500">{new Date(order.createdAt).toLocaleString('uz-UZ')}</p><details className="mt-2 text-xs text-gray-400"><summary className="cursor-pointer">Status tarixi</summary><ul className="mt-2 space-y-1">{order.history.map((entry) => <li key={entry.id}>{statusLabels[entry.status]} · {new Date(entry.createdAt).toLocaleString('uz-UZ')}</li>)}</ul></details></td><td className="p-4"><p className="font-semibold text-white">{order.customerName}</p><a href={`tel:${order.phone}`} className="text-xs text-sky-400">{order.phone}</a><p className="mt-1 text-xs text-gray-500">{order.region}</p></td><td className="max-w-xs p-4"><ul className="space-y-1 text-xs text-gray-300">{order.items.map((item) => <li key={item.id}><span className="font-mono text-gray-500">{item.skuSnapshot}</span> — {item.nameSnapshotUz}, {item.quantity} {item.saleUnit.toLowerCase()}</li>)}</ul></td><td className="p-4 font-semibold text-white">{order.total == null ? 'So‘rov' : `${Number(order.total).toLocaleString('uz-UZ')} UZS`}</td><td className="p-4"><select disabled={updating === order.id} value={order.assignedToId || ''} onChange={(event) => void update(order.id, { assignedToId: event.target.value || null })} className="max-w-44 rounded-lg border border-white/20 bg-black px-3 py-2 text-xs text-white"><option value="">Biriktirilmagan</option>{managers.map((manager) => <option key={manager.id} value={manager.id}>{manager.name || manager.email}</option>)}</select></td><td className="p-4"><span className={`mb-2 block w-fit rounded-full px-2.5 py-1 text-xs ${statusTone[order.status]}`}>{statusLabels[order.status]}</span><select disabled={updating === order.id} value={order.status} onChange={(event) => void update(order.id, { status: event.target.value as Status })} className="rounded-lg border border-white/20 bg-black px-3 py-2 text-xs text-white">{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></td></tr>)}
    </tbody></table></div></div>
  </div>;
}
