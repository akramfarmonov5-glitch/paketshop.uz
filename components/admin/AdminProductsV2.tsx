...
export default function AdminProductsV2() {
  ...
  return <div className="space-y-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl font-bold">B2B mahsulotlar</h2><p className="text-sm text-slate-500">SKU, qadoq, korobka va narx birligi</p></div><div className="flex gap-2"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="SKU yoki nom" className="rounded-xl border border-slate-200 bg-white px-3 py-2" /><button onClick={() => void load()} className="rounded-xl border border-slate-200 p-2.5"><RefreshCw size={18} /></button></div></div>
    ...
    <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-4"><div className="mb-4 flex items-center justify-between"><h3 className="font-semibold">{editing ? `${editing.sku} ni tahrirlash` : 'Yangi mahsulot'}</h3>{editing && <button type="button" onClick={reset} className="p-1 text-slate-500"><X size={18} /></button>}</div><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
      <input required value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value.toUpperCase() })} placeholder="SKU" className="rounded-xl border border-slate-200 bg-white px-3 py-2.5" /><input required value={form.nameUz} onChange={(e) => setForm({ ...form, nameUz: e.target.value })} placeholder="O‘zbekcha nom" className="rounded-xl border border-slate-200 bg-white px-3 py-2.5" /><input required value={form.nameRu} onChange={(e) => setForm({ ...form, nameRu: e.target.value })} placeholder="Русское название" className="rounded-xl border border-slate-200 bg-white px-3 py-2.5" /><select required value={form.categoryId} onChange={(e) => setForm({ ...form, categoryId: e.target.value })} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5"><option value="">Kategoriya</option>{categories.filter((category) => (category as CategoryOption & { active?: boolean }).active !== false).map((category) => <option key={category.id} value={category.id}>{categoryName.get(category.id)}</option>)}</select>
    ...
    <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white">{loading ? <div className="p-10 text-center"><Loader2 className="mx-auto animate-spin" /></div> : <table className="w-full min-w-[800px] text-left text-sm"><thead className="bg-slate-50 text-slate-500"><tr><th className="p-3">SKU</th><th className="p-3">Mahsulot</th><th className="p-3">Kategoriya</th><th className="p-3">Qadoq / korobka</th><th className="p-3">Narx</th><th className="p-3">Variant / daraja</th><th className="p-3">Holat</th><th className="p-3"></th></tr></thead><tbody className="divide-y divide-slate-100">{products.map((product) => <tr key={product.id}><td className="p-3 font-mono">{product.sku}</td><td className="p-3 font-medium">{product.translations.find((translation) => translation.locale === 'uz')?.name}</td><td className="p-3 text-slate-500">{categoryName.get(product.categoryId)}</td><td className="p-3">{product.unitsPerPack} / {product.unitsPerCarton}</td><td className="p-3">{product.publicPrice == null ? 'So‘rov' : Number(product.publicPrice).toLocaleString('uz-UZ')}</td><td className="p-3 text-slate-500">{product.variants?.length || 0} / {product.priceTiers?.length || 0}</td><td className="p-3 text-slate-500">{product.availabilityStatus}</td><td className="p-3"><div className="flex gap-2"><button onClick={() => edit(product)} className="p-2 text-blue-400" aria-label="Tahrirlash"><Edit3 size={17} /></button><button onClick={() => void archive(product.id)} className="p-2 text-red-400" aria-label="Arxivlash"><Archive size={17} /></button></div></td></tr>)}</tbody></table>}</div>
  </div>;
}
